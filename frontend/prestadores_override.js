...
function prestStatusHtml(ativo){return ativo?'<span style="color:#2fbf2f;font-size:14px;line-height:1;">â—</span>':'<span style="color:#c0c8d2;font-size:14px;line-height:1;">â—</span>'}
...
const PREST_AGENDA_VISUALIZACAO_PADRAO=["NÃºmero do paciente","Nome do paciente","Fone 1","Fone 2","Sala"];
...
function prestTiposPrestadorPadrao(){return[{codigo:"01",descricao:"CirurgiÃ£o dentista"},{codigo:"02",descricao:"Clínica odontolÃ³gica"},{codigo:"03",descricao:"Clínica ortodÃ´ntica"},{codigo:"04",descricao:"Clínica radiolÃ³gica"},{codigo:"05",descricao:"Perito"}]}
function prestSexosPadrao(){return["Masculino","Feminino"]}
function prestCbosPadrao(){return["Cir.Dentista em Geral","Cir.Dentista (saÃºde pÃºblica)","Cir.Dentista (traumatologia buco maxilo facial)","Cir.Dentista (endodontia)","Cir.Dentista (ortodontia)","Cir.Dentista (patologia bucal)","Cir.Dentista (pediatria)","Cir.Dentista (prÃ³tese)","Cir.Dentista (radiologia)","Cir.Dentista (periodontia)"]}
function prestUfCroPadrao(){return PREST_UFS_BR.slice()}
function prestNovoItemBase(){return{id:Date.now(),codigo:prestNovoCodigo(),nome:"",apelido:"",tipo_prestador:"CirurgiÃ£o dentista",inicio:prestHojeBr(),termino:"",ativo:true,executa_procedimento:true,cro:"",uf_cro:"",cpf:"",rg:"",inss:"",ccm:"",contrato:"",cnes:"",cbos:"Cir.Dentista em Geral",nascimento:"",sexo:"",estado_civil:"",prefixo:"",inclusao:prestHojeBr(),alteracao:prestHojeBr(),id_interno:String(Date.now()).slice(-6),fone1:"",fone1_tipo:"Residencial",fone2:"",fone2_tipo:"Comercial",email:"",homepage:"",logradouro_tipo:"",endereco:"",numero:"",complemento:"",bairro:"",cidade:"SÃ£o JosÃ© do Rio Preto",cep:"",uf:"SP",banco:"",agencia:"",conta:"",nome_conta:"",modo_pagamento:"",faculdade:"",formatura:"",alerta_agendamentos:"",especialidades_exec:[],observacoes:"",especialidade:""}}
...
function prestEspecialidadesPadrao(){return["Cirurgia","DentÃ­stica","DiagnÃ³stico","Endodontia","EstÃ©tica","Gerais","HarmonizaÃ§Ã£o Facial","Implantodontia","Odontopediatria","Ortodontia","Periodontia","PrevenÃ§Ã£o","PrÃ³tese","Radiologia"]}
...
function prestAuxPadrao(tipo){switch(String(tipo||"").trim()){case"Tipos de contato":return["Comercial 1","Comercial 2","Comercial 3","Residencial","Fax","Celular","Recado"];case"Bancos":return["Banco do Brasil","Caixa EconÃ´mica Federal","Bradesco","ItaÃº","Santander"];case"Estado civil":return["Solteiro(a)","Casado(a)","Divorciado(a)","ViÃºvo(a)"];case"Prefixo pessoais":return["Dr.","Dra.","Sr.","Sra."];case"Tipos de pagamento":return["DepÃ³sito","PIX","TransferÃªncia","Cheque"];case"Cidade":return["SÃ£o JosÃ© do Rio Preto","Mirassol","Cedral","BÃ¡lsamo"];case"Tipos de logradouro":return["Rua","Avenida","Alameda","Travessa"];default:return[]}}
...
if(/Endere.o residencial:/i.test(txt))lbl.textContent="EndereÃ§o residencial:";
if(/^N.{0,2}:$/.test(txt))lbl.textContent="NÂº:";
});
...
const canonicalCidade=['<option value=""></option>','<option>SÃ£o JosÃ© do Rio Preto</option>','<option>Mirassol</option>','<option>Cedral</option>','<option>BÃ¡lsamo</option>'].join("");
...
async function prestCarregarCombosAuxiliares(base){if(!prestCfg?.modal)return;const m=prestCfg.modal;const tiposTelefonePrestador=["Residencial","Comercial","Celular","Recado"];const [bancos,estados,prefixos,pagamentos,cidades,bairros,logradouros]=await Promise.all([prestCarregarAuxTipoLista("Bancos"),prestCarregarAuxTipoLista("Estado civil"),prestCarregarAuxTipoLista("Prefixo pessoais"),prestCarregarAuxTipoLista("Tipos de pagamento"),prestCarregarAuxTipoLista("Cidade"),prestCarregarAuxTipoLista("Bairro"),prestCarregarAuxTipoLista("Tipos de logradouro")]);prestPreencherComboLista(m.foneTipo1,tiposTelefonePrestador,base.fone1_tipo||"Residencial",false);prestPreencherComboLista(m.foneTipo2,tiposTelefonePrestador,base.fone2_tipo||"Comercial",false);prestPreencherComboLista(m.estadoCivil,estados,base.estado_civil||"",true);prestPreencherComboLista(m.prefixo,prefixos,base.prefixo||"",true);prestPreencherComboLista(m.banco,bancos,base.banco||"",true);prestPreencherComboLista(m.modoPagamento,pagamentos,base.modo_pagamento||"",true);prestPreencherComboLista(m.cidade,cidades,base.cidade||"SÃ£o JosÃ© do Rio Preto",true);prestPreencherComboLista(m.bairro,bairros,base.bairro||"",true);prestPreencherComboLista(m.logradouroTipo,logradouros,base.logradouro_tipo||"",true);prestPreencherComboLista(m.uf,PREST_UFS_BR,base.uf||"SP",true)}
...
prestPreencherComboTiposPrestador(m.tipo,base.tipo_prestador||"CirurgiÃ£o dentista");
await prestCarregarCombosAuxiliares(base);
await prestCarregarCombosPrincipal(base);
m.inicio.value=String(base.inicio||prestHojeBr());
m.termino.value=String(base.termino||"");
...
m.nascimento.value=String(base.nascimento||"");
m.sexo.value=String(base.sexo||"");
m.inclusao.value=String(base.inclusao||"");
m.alteracao.value=String(base.alteracao||"");
...
function prestAgendaFontePreviewSync(){if(!prestCfg?.agendaFonteModal)return;const m=prestCfg.agendaFonteModal;const styleValue=String(m.style.value||"Regular");const isBold=styleValue==="Negrito"||styleValue==="OblÃ­quo e negrito";const isItalic=styleValue==="OblÃ­quo"||styleValue==="OblÃ­quo e negrito";m.sample.style.fontFamily=String(m.family.value||"MS Sans Serif");m.sample.style.fontSize=`${Number(m.size.value||8)}px`;m.sample.style.fontWeight=isBold?"700":"400";m.sample.style.fontStyle=isItalic?"italic":"normal";m.sample.style.textDecoration=[m.underline.checked?"underline":"",m.strike.checked?"line-through":""].filter(Boolean).join(" ")||"none";m.sample.style.color=String(m.color.value||"#000000");m.colorSwatch.style.background=String(m.color.value||"#000000");m.familyInput.value=m.family.value;m.styleInput.value=m.style.value;m.sizeInput.value=m.size.value}
function prestAgendaFonteAbrir(){prestAgendaEnhanceApresentacao();const s=prestCfg.agenda.apresFonteState||{family:"MS Sans Serif",size:8,bold:false,italic:false,underline:false,strike:false,color:"#000000",script:"Ocidental"};const m=prestCfg.agendaFonteModal;m.family.value=s.family;m.style.value=s.bold&&s.italic?"OblÃ­quo e negrito":s.bold?"Negrito":s.italic?"OblÃ­quo":"Regular";m.size.value=String(s.size);m.strike.checked=!!s.strike;m.underline.checked=!!s.underline;m.color.value=s.color;m.script.value=s.script||"Ocidental";prestAgendaFontePreviewSync();m.backdrop.classList.remove("hidden")}
function prestAgendaFonteSalvar(){if(!prestCfg?.agendaFonteModal)return;const styleValue=String(prestCfg.agendaFonteModal.style.value||"Regular");prestCfg.agenda.apresFonteState={family:String(prestCfg.agendaFonteModal.family.value||"MS Sans Serif"),size:Number(prestCfg.agendaFonteModal.size.value||8),bold:styleValue==="Negrito"||styleValue==="OblÃ­quo e negrito",italic:styleValue==="OblÃ­quo"||styleValue==="OblÃ­quo e negrito",underline:!!prestCfg.agendaFonteModal.underline.checked,strike:!!prestCfg.agendaFonteModal.strike.checked,color:String(prestCfg.agendaFonteModal.color.value||"#000000"),script:String(prestCfg.agendaFonteModal.script.value||"Ocidental")};prestAgendaApresPreviewSync();prestAgendaFonteFechar()}
function prestAgendaVisualizacaoCampos(){return["NÃºmero do paciente","NÃºmero do prontuÃ¡rio","Nome do paciente","MatrÃ­cula","ConvÃªnio","Tabela","Fone 1","Fone 2","Fone 3","Sala"]}
...
pane.innerHTML=`<div class="prest-agenda-apres-wrap"><div class="prest-agenda-apres-box"><h4>Cor de fundo</h4><div class="prest-agenda-apres-field"><label for="prest-agenda-apres-particular">Pacientes particulares:</label><span class="prest-agenda-apres-swatch" id="prest-agenda-swatch-particular"></span><div><select id="prest-agenda-apres-particular">${colorOptions}</select></div></div><div class="prest-agenda-apres-field"><label for="prest-agenda-apres-convenio">Pacientes de convÃªnio:</label><span class="prest-agenda-apres-swatch" id="prest-agenda-swatch-convenio"></span><div><select id="prest-agenda-apres-convenio">${colorOptions}</select></div></div><div class="prest-agenda-apres-field" style="margin-bottom:0"><label for="prest-agenda-apres-compromisso">Compromissos:</label><span class="prest-agenda-apres-swatch" id="prest-agenda-swatch-compromisso"></span><div><select id="prest-agenda-apres-compromisso">${colorOptions}</select></div></div></div><div class="prest-agenda-apres-box"><h4>Tipo de letra (fonte)</h4><div class="prest-agenda-apres-font-top"><button id="prest-agenda-apres-fonte" class="materiais-btn" type="button"><img src="/desktop-assets/editar.png" alt="">Altera letra...</button></div><div id="prest-agenda-preview-particular" class="prest-agenda-apres-preview">Particular</div><div id="prest-agenda-preview-convenio" class="prest-agenda-apres-preview">ConvÃªnio</div><div id="prest-agenda-preview-compromisso" class="prest-agenda-apres-preview" style="margin-bottom:0">Compromisso</div></div></div>`;
...
let t=String(txt||"");
t=t
.replaceAll("Conv?nio","ConvÃªnio")
.replaceAll("C?digo","CÃ³digo")
.replaceAll("In?cio","InÃ­cio")
.replaceAll("Vig?ncia","Vigência")
.replaceAll("Inclus?o","InclusÃ£o")
.replaceAll("Altera??o","AlteraÃ§Ã£o")
.replaceAll("Observa??es","Observações")
.replaceAll("ConvÃƒÂªnio","ConvÃªnio")
.replaceAll("CÃƒÂ³digo","CÃ³digo")
.replaceAll("InÃƒÂ­cio","InÃ­cio")
.replaceAll("VigÃƒÂªncia","Vigência")
.replaceAll("InclusÃƒÂ£o","InclusÃ£o")
.replaceAll("AlteraÃƒÂ§ÃƒÂ£o","AlteraÃ§Ã£o")
.replaceAll("ObservaÃƒÂ§ÃƒÂµes","Observações");
return t;
};
...
async function prestComAbrirModal(modo){const item=modo==="editar"?prestComAtual():null;if(modo==="editar"&&!item){window.alert("Selecione um fator de comiss?o.");return}prestComEnsureUI();const genericos=await prestComCarregarGenericos();const m=prestComCfg.modal;const base=item?JSON.parse(JSON.stringify(item)):{id:Date.now(),vigencia:prestHojeBr(),prestador_id:prestComPrestadorAtual()==="__todos__"?String(prestSelecionado()?.id||"-1"):prestComPrestadorAtual(),convenio_row_id:prestComConvenioAtual()==="__todos__"?"":prestComConvenioAtual(),especialidade:prestEspecialidadesTexto(prestSelecionado())||"Gerais",procedimento_generico_id:"",tipo_repasse:"% sobre valor",repasse:"0,00",inclusao:prestHojeBr(),alteracao:prestHojeBr()};m.modo=modo;m.editId=item?Number(item.id||0):null;m.title.textContent=modo==="editar"?"Altera fator de comiss?o":"Novo fator de comiss?o";m.cboConvenio.innerHTML=prestCredConvenios.map(row=>`<option value="${esc(String(row.row_id||row.id||""))}">${esc(String(row.nome||""))}</option>`).join("");m.cboPrestador.innerHTML=prestadoresCache.map(row=>`<option value="${esc(String(row.id||""))}">${esc(String(row.nome||""))}</option>`).join("");m.cboEspecialidade.innerHTML=prestComEspecialidadesLista([String(base.especialidade||"").trim()]).map(row=>`<option value="${esc(String(row))}">${esc(String(row))}</option>`).join("");m.cboProcedimento.innerHTML=['<option value=""></option>',...genericos.map(row=>`<option value="${esc(String(row.id||""))}">${esc(String(row.descricao||row.nome||row.codigo||""))}</option>`)].join("");m.cboTipoRepasse.innerHTML=['<option>% sobre valor</option>','<option>Valor fixo</option>'].join("");m.cboPrestador.value=String(base.prestador_id||"");m.cboConvenio.value=String(base.convenio_row_id||"");m.cboEspecialidade.value=String(base.especialidade||"");m.cboProcedimento.value=String(base.procedimento_generico_id||"");m.vigencia.value=String(base.vigencia||prestHojeBr());m.cboTipoRepasse.value=String(base.tipo_repasse||"% sobre valor");m.repasse.value=String(base.repasse||"0,00");m.inclusao.value=String(base.inclusao||prestHojeBr());m.alteracao.value=String(base.alteracao||prestHojeBr());m.backdrop.classList.remove("hidden")}
async function prestComSalvarModal(){if(!prestComCfg?.modal)return;const m=prestComCfg.modal;const convenioId=String(m.cboConvenio.value||"").trim();const prestadorId=String(m.cboPrestador.value||"").trim();if(!convenioId||!prestadorId){window.alert("Preencha conv?nio e prestador.");return}const payload={vigencia:String(m.vigencia.value||prestHojeBr()).trim(),prestador_row_id:Number(prestadorId||0)>0?Number(prestadorId||0):null,convenio_row_id:Number(convenioId||0),especialidade:String(m.cboEspecialidade.value||"").trim(),procedimento_generico_id:Number(m.cboProcedimento.value||0)||null,tipo_repasse:String(m.cboTipoRepasse.value||"% sobre valor"),repasse:String(m.repasse.value||"0,00").trim()||"0,00"};try{const editId=Number(m.editId||0);const endpoint=m.modo==="editar"?`/cadastros/prestadores/comissoes/${editId}`:"/cadastros/prestadores/comissoes";const method=m.modo==="editar"?"PUT":"POST";const{res,data}=await requestJson(method,endpoint,payload,true);if(!res.ok)throw new Error(String(data?.detail||"Falha ao gravar fator de comiss?o."));const item=data||{};const idx=prestComItens.findIndex(row=>Number(row.id||0)===Number(item.id||0));if(idx>=0)prestComItens[idx]=item;else prestComItens.push(item);prestComSelId=Number(item.id||0)||null;prestComRender();prestComFecharModal();footerMsg.textContent=`Fator de comiss?o de '${item.prestador_nome||""}' gravado.`}catch(err){window.alert(err?.message||"N?o foi poss?vel gravar o fator de comiss?o.")}}
async function prestComExcluirSelecionado(){const item=prestComAtual();if(!item){window.alert("Selecione um fator de comiss?o.");return}if(!window.confirm(`Deseja eliminar o fator de comiss?o de ${item.prestador_nome} ?`))return;try{const{res,data}=await requestJson("DELETE",`/cadastros/prestadores/comissoes/${Number(item.id||0)}`,undefined,true);if(!res.ok)throw new Error(String(data?.detail||"Falha ao eliminar fator de comiss?o."));prestComItens=prestComItens.filter(row=>Number(row.id||0)!==Number(item.id||0));prestComSelId=null;prestComRender();footerMsg.textContent=`Fator de comiss?o de '${item.prestador_nome}' eliminado.`}catch(err){window.alert(err?.message||"N?o foi poss?vel eliminar o fator de comiss?o.")}}
...
function prestComNormalizarTextos(){if(!prestComCfg)return;const panel=prestComCfg.panel;const title=panel?.querySelector(".panel-title");if(title)title.textContent="Configura fatores de comissÃ£o";const l1=panel?.querySelector('label[for="prest-com-convenio"]');if(l1)l1.textContent="ConvÃªnio:";const l2=panel?.querySelector('label[for="prest-com-prestador"]');if(l2)l2.textContent="Prestador:";const heads=panel?.querySelectorAll("thead th");if(heads&&heads.length>=5){heads[0].textContent="Vigência";heads[1].textContent="Prestador";heads[2].textContent="ConvÃªnio";heads[3].textContent="Especialidade";heads[4].textContent="Repasse"}if(prestComCfg.btnNovo)prestComCfg.btnNovo.lastChild.textContent="Novo fator de comissÃ£o...";if(prestComCfg.btnEditar)prestComCfg.btnEditar.lastChild.textContent="Altera...";if(prestComCfg.btnExcluir)prestComCfg.btnExcluir.lastChild.textContent="Elimina";if(prestComCfg.btnFechar)prestComCfg.btnFechar.lastChild.textContent="Fecha"}
...
function prestComNormalizarTextos(){if(!prestComCfg)return;const panel=prestComCfg.panel;const title=panel?.querySelector(".panel-title");if(title)title.textContent="Configura fatores de comissÃ£o";const l1=panel?.querySelector('label[for="prest-com-convenio"]');if(l1)l1.textContent="ConvÃªnio:";const l2=panel?.querySelector('label[for="prest-com-prestador"]');if(l2)l2.textContent="Prestador:";const heads=panel?.querySelectorAll("thead th");if(heads&&heads.length>=5){heads[0].textContent="Vigência";heads[1].textContent="Prestador";heads[2].textContent="ConvÃªnio";heads[3].textContent="Especialidade";heads[4].textContent="Repasse"}if(prestComCfg.btnNovo)prestComCfg.btnNovo.lastChild.textContent="Novo fator de comissÃ£o...";if(prestComCfg.btnEditar)prestComCfg.btnEditar.lastChild.textContent="Altera...";if(prestComCfg.btnExcluir)prestComCfg.btnExcluir.lastChild.textContent="Elimina";if(prestComCfg.btnFechar)prestComCfg.btnFechar.lastChild.textContent="Fecha";const m=prestComCfg.modal;if(!m)return;const setLbl=(id,txt)=>{const el=document.getElementById(id);const lbl=el?.closest("div")?.querySelector("label");if(lbl)lbl.textContent=txt};setLbl("prest-com-modal-convenio","ConvÃªnio:");setLbl("prest-com-modal-prestador","Prestador:");setLbl("prest-com-modal-especialidade","Especialidade:");setLbl("prest-com-modal-procedimento","Procedimento genÃ©rico:");setLbl("prest-com-modal-vigencia","InÃ­cio da vigÃªncia:");setLbl("prest-com-modal-tipo-repasse","Tipo de repasse:");setLbl("prest-com-modal-repasse","Valor de repasse:");setLbl("prest-com-modal-inclusao","InclusÃ£o:");setLbl("prest-com-modal-alteracao","AlteraÃ§Ã£o:")}
...
async function prestComAbrirModal(modo){const item=modo==="editar"?prestComAtual():null;if(modo==="editar"&&!item){window.alert("Selecione um fator de comissÃ£o.");return}prestComEnsureUI();prestComApplyFineLayout();prestComNormalizarTextos();const[genericos,especialidades]=await Promise.all([prestComCarregarGenericos(),prestComCarregarEspecialidades()]);const m=prestComCfg.modal;const base=item?JSON.parse(JSON.stringify(item)):{id:Date.now(),vigencia:prestHojeBr(),prestador_id:prestComPrestadorAtual()==="__todos__"?String(prestSelecionado()?.id||"-1"):prestComPrestadorAtual(),convenio_row_id:prestComConvenioAtual()==="__todos__"?"":prestComConvenioAtual(),especialidade_row_id:null,especialidade:prestEspecialidadesTexto(prestSelecionado())||"Gerais",procedimento_generico_id:"",tipo_repasse_codigo:1,tipo_repasse:"% sobre valor",repasse:"0,00",inclusao:prestHojeBr(),alteracao:prestHojeBr()};m.modo=modo;m.editId=item?Number(item.id||0):null;m.title.textContent=modo==="editar"?"Altera fator de comissÃ£o":"Novo fator de comissÃ£o";m.cboConvenio.innerHTML=prestCredConvenios.map(row=>`<option value="${esc(String(row.row_id||row.id||""))}">${esc(String(row.nome||""))}</option>`).join("");m.cboPrestador.innerHTML=prestadoresCache.map(row=>`<option value="${esc(String(row.id||""))}">${esc(String(row.nome||""))}</option>`).join("");const espOpts=(Array.isArray(especialidades)&&especialidades.length?especialidades:[]).map(row=>({id:Number(row.id||0)||0,nome:String(row.nome||"").trim()})).filter(row=>row.id>0&&row.nome);let espAtualId=Number(base.especialidade_row_id||0)||0;let espAtualNome=String(base.especialidade||"").trim();if(!espAtualId&&espAtualNome){const achado=espOpts.find(row=>row.nome.toLowerCase()===espAtualNome.toLowerCase());if(achado)espAtualId=achado.id}const espHtml=['<option value=""></option>',...espOpts.map(row=>`<option value="${row.id}">${esc(row.nome)}</option>` )];if(espAtualNome&&!espAtualId&&!espOpts.some(row=>row.nome.toLowerCase()===espAtualNome.toLowerCase()))espHtml.push(`<option value="__legacy__" selected>${esc(espAtualNome)}</option>`);m.cboEspecialidade.innerHTML=espHtml.join("");m.cboProcedimento.innerHTML=['<option value=""></option>',...genericos.map(row=>`<option value="${esc(String(row.id||""))}">${esc(String(row.descricao||row.nome||row.codigo||""))}</option>`)].join("");m.cboTipoRepasse.innerHTML='<option value="1">% sobre valor</option><option value="2">Valor fixo</option>';m.cboPrestador.value=String(base.prestador_id||"");m.cboConvenio.value=String(base.convenio_row_id||"");if(espAtualId)m.cboEspecialidade.value=String(espAtualId);m.cboProcedimento.value=String(base.procedimento_generico_id||"");m.vigencia.value=String(base.vigencia||prestHojeBr());m.cboTipoRepasse.value=String(Number(base.tipo_repasse_codigo||0)||((String(base.tipo_repasse||"").includes("%"))?1:2));m.repasse.value=String(base.repasse||"0,00");m.inclusao.value=String(base.inclusao||prestHojeBr());m.alteracao.value=String(base.alteracao||prestHojeBr());prestComNormalizarTextos();m.backdrop.classList.remove("hidden")}
...
// PATCH Agenda da semana: habilita seleÃ§Ã£o/ediÃ§Ã£o de agendamento existente
// sem alterar a estrutura-base do mÃ³dulo legado.
if(typeof agendaSemanaRenderEventos==="function"){
  ...
  const agendaSemanaBuscarOcupanteIntervalo=(dataIso,inicioMs,fimMs)=>{
  ...
  const agendaSemanaIndiceDiaVigente=()=>{
    ...
    const focusIso=agendaSemanaState?.focusDate?agendaSemanaToIsoDate(agendaSemanaState.focusDate):"";
    let idx=focusIso?dias.findIndex(d=>agendaSemanaToIsoDate(d)===focusIso):-1;
    if(idx<0){
      const hojeIso=agendaSemanaToIsoDate(new Date());
      idx=dias.findIndex(d=>agendaSemanaToIsoDate(d)===hojeIso);
    }
    return idx;
  };
  ...
  const agendaSemanaGarantirDialogoExclusao=()=>{
    ...
    if(!document.getElementById("agenda-semana-delete-style")){
      ...
      style.textContent=`
        .agenda-semana-delete-backdrop{
          position:fixed;inset:0;z-index:2600;background:rgba(0,0,0,.30);
          display:grid;place-items:center
        }
        .agenda-semana-delete-backdrop.hidden{display:none}
        .agenda-semana-delete-modal{
          width:min(420px,92vw);border:1px solid #8fa3ba;background:#ececec;
          box-shadow:2px 2px 10px rgba(0,0,0,.3);padding:12px;box-sizing:border-box;
          font:12px Tahoma,sans-serif
        }
        .agenda-semana-delete-title{font:700 12px Tahoma,sans-serif;margin-bottom:8px}
        .agenda-semana-delete-msg{line-height:1.35;margin-bottom:12px}
        .agenda-semana-delete-actions{display:flex;justify-content:flex-end;gap:8px}
        .agenda-semana-delete-actions button{
          min-width:84px;height:28px;border:1px solid #9ea7b4;background:#ececec;
    ...
    backdrop.innerHTML=`
      <div class="agenda-semana-delete-modal" role="dialog" aria-modal="true" aria-labelledby="agenda-semana-delete-title">
        <div id="agenda-semana-delete-title" class="agenda-semana-delete-title">Confirma exclusão</div>
        <div id="agenda-semana-delete-msg" class="agenda-semana-delete-msg"></div>
        <div class="agenda-semana-delete-actions">
          <button id="agenda-semana-delete-cancelar" type="button">Cancelar</button>
          <button id="agenda-semana-delete-excluir" type="button">Excluir</button>
        </div>
      </div>
    `;
  ...
  const agendaSemanaAbrirModalEditar=async(item)=>{
    ...
    if(agendaLegado.modalNome)agendaLegado.modalNome.focus();
  };
